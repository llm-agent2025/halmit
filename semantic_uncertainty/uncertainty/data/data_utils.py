...
def load_ds(dataset_name, seed, add_options=None):
    ...
    if dataset_name == "squad":
        ...
        scratch_dir = 'scratch'
        path = f"{scratch_dir}/{user}/uncertainty/data/bioasq/training11b.json"
        ...
        # scratch_dir = os.getenv('SCRATCH_DIR', '.')
        scratch_dir = 'scratch'
        path = f"{scratch_dir}/{user}/uncertainty/data/bioasq/tree.json"
        with open(path, "rb") as file:
            data = json.load(file)
        ...
        for question in questions:
            dataset_dict["question"].append(question["prompt"])

            dataset_dict["answers"].append({
                "text": question["answer"],
                "answer_start": [0] * len(question["answer"])
            })
            dataset_dict["id"].append(question["ID"])
