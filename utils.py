...
from actionconfig import ActionConfig
...
def get_parser(stages=['generate', 'compute'], default_prompt = "Did humans really land on the moon in 1969?"):
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            ...
            help="prompt to compute")
        parser.add_argument(
            "--id", type=str, default="test",
            help="prompt to compute")
        parser.add_argument(
        ...
        parser.add_argument(
            "--num_few_shot", type=int, default=1,
...
def construct_action_prompt(action, question, topic):
    actionconfig = getattr(ActionConfig, action)[0]
    prefix = actionconfig.get('prefix', '')
    prompt = actionconfig.get('prompt', '')
    input = ''.join([prefix, prompt, question])
    construct_prompt = input.replace("{topic_non}", topic)
    return construct_prompt

def construct_fewshot_prompt_from_indices(example_indices, brief, brief_always, make_prompt):
    ...
    question = example_indices
    prompt = prompt + make_prompt(question, brief, brief_always)
...
def get_make_prompt(args):
    if args.prompt_type == 'default':
        def make_prompt(question, brief, brief_always):
            prompt = ''
            if brief_always:
                prompt += brief
...
def save(object, file, directory):
